...
exports.getBounty = async (req, res) => {
    try {
      ...
      const bounties = await Bounty.find({
        ...
        .sort({ createdAt: -1 })
        .populate("createdBy", "name profileImage") // also fetch profileImage of creator
        .populate("eligibleCollege", "name");
  
      // 3. Format bounty info
      const formattedBounties = bounties.map((bounty) => {
        ...
        // Use creator's image if not current user
        const profileImage =
...
exports.enrollInBounty = async (req, res) => {
    try {
      ...
      const alreadyInQueue = await BountyQueue.findOne({ user: userId, bounty: bountyId });
      if (alreadyInQueue) {
        return res.status(400).json({ success: false, message: "Already enrolled" });
      }
  
      await BountyQueue.create({ bounty: bountyId, user: userId });
